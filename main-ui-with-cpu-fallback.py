...
def process_text(text_chunks, ref_file, ref_text, model, device):
    ...
    for chunk in text_chunks:
        with torch_gc():
            ...
            inference_context = torch.cuda.amp.autocast() if torch.cuda.is_available() else torch.inference_mode()
...
async def startup_event():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            tts_model.ema_model = tts_model.ema_model.cpu()
            tts_model.device = torch.device('cpu')
...
async def health_check():
    ...
    return {
        ...
        "using_mixed_precision": torch.cuda.is_available(),  # Only true when using GPU
